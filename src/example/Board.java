...
//				trainCards[i] = new TrainCard(colors[9]); // Rainbow // with jokers
	...
	*/
	...
	public void REMOVEDcheckConnected(Town _townA, Town _townB){ //pass in player as well
			...
			for(int i = 0; i < _townA.getConnections().size(); i++){
	...
	public boolean _areConnected(Town _townA, Town _townB /*, Player player*/){
		...
		System.out.println("Checking if "+_townA.getName()+" and "+_townB.getName()+" are connected");
		System.out.println(_townA.getName()+" has "+_townA.getConnections().size()+" connections");
	...
	for(int i = 0; i < current.getConnections().size(); i++){
		...
		for(int j = 0; j < current.getConnections().size(); j++){
			 ...
			 if(current.getConnection(j).getIsTaken()){
					...
					current.getConnection(j).setIsVisited(true);

					System.out.println("Current before recursion: "+current.getName());
					
					returnValue = returnValue || _areConnected(current, _townB);
 ...
 for(int i = 0; i < current.getConnections().size(); i++){
	...
	void setAllUnVisited(){
		...
		for(int i = 0; i < connections.size(); i++){
			...
			connections.get(i).setIsVisited(false);
